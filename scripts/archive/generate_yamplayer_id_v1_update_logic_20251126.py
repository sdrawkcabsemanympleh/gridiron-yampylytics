...
def build_unified_players_table(con: duckdb.DuckDBPyConnection, base_dir: Path) -> None:
    ...
    INSERT-only approach with tiered OR matching to avoid type casting issues.

    Matching tiers:
    - TIER 1: ANY official ID match (gsis_id, pfr_id)
    - TIER 2: merge_name + birthdate (cast to VARCHAR for comparison)
